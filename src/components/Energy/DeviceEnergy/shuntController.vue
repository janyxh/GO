<template>
  <el-col class="lis" :class="item.value1 ? 'off' : 'on'">
    ...
    <el-row class="powerShow">
      <el-col :span="12" class="powerLeft">
        <p>{{item.workPower}}kWh</p>
        本月用电量
      </el-col>
      <el-col :span="12" class="powerRight">
        <p>{{item.power}}W</p>
        当前功率
      </el-col>
    </el-row>
...
<script>
  import { controlDevice, checkKKPower } from '../../../api/api';
  export default {
    name: "shuntController",
    props: ['item','getList'],
    data: function data() {
      return {}
    },
      ...
      openDevice(item,getList){
        ...
        checkKKPower(para).then((res) =>{
          if(res.code == 200){
            ...
            if(res.data.enable_shutdown == 'True'){
...
<style lang="scss" scoped>
  .lis {
    ...
    &.on{
      ...
      .picWrap{
        .picture{
          background: url("../../../assets/img/icon_box_p.png") no-repeat center center;
        }
      }
    }
    &.off{
      ...
      .picWrap{
        .picture{
          background: url("../../../assets/img/icon_box.png") no-repeat center center;
        }
      }
      .switch{
        .switchBtn{
